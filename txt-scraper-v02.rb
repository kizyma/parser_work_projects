...
def download(link, email)
  ...
  if HTTParty.get(link).to_s.force_encoding("ISO-8859-1").encode("utf-8", replace: nil).downcase.gsub(/[^a-z0-9]/, '').include? "systeminformationreportwrittenat"
    #if the condition above is true, creating a new file with a timestamp and a customer's email in the name
